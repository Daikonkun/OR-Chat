...
- [ ] `HTTP-Referer` header is read from `OPENROUTER_HTTP_REFERER` env var (default: `http://localhost:8888`)
- [ ] `X-Title` header is read from `OPENROUTER_X_TITLE` env var (default: `OpenRouter Local Wrapper`)
- [ ] Missing env vars do not cause errors — defaults are used when env vars are unset or empty
- [ ] Header values are correctly forwarded to OpenRouter API calls (verified via logs or test)
